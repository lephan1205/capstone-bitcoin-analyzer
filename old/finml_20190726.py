...
def dollar_sampling(df, dollars_per_bar = 2e6):
    ...
    data_cm_dollar = df.assign(cmDollar=data['foreignNotional'].cumsum())
...
def labeling(df, h = pd.Timedelta(hours=1), m = [2, 2]):
    ...
    df = df.assign(retp1 = fn.get_return(df.close)).dropna()
    
    # add thresholds and vertical barrier (t1) columns
    df = df.assign(threshold=fn.get_volatility(df.close, delta=h), 
                             t1=fn.get_verticals(df, delta=h)).dropna()
